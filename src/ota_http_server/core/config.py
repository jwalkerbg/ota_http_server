    ...
    def merge_cli_options(self, config_cli: argparse.Namespace | None = None) -> ConfigDict:    # pylint: disable=too-many-branches
        ...
        if config_cli:
            ...
            if config_cli.command == 'runserver':
                ...
                if getattr(config_cli, "jwt_user_audience", None) is not None:
...
Environment variables:
  ...
  OTA_JWT_USER_EXPIRY_SECONDS REST API user JWT expiry time in seconds (default 1800), can be overridden by --jwt-user-expiry CLI option
...
"""
  ...
  'www-dir' is the root directory of the http server.""")
    ...
    # db rollback#
    rollback_parser = db_subparsers.add_parser(name="rollback", help="Execute rollback of the last migration")
